...
enum class EVeloxTypeFromC {
    INVALID,
    _schar, _short, _long, _longlong, _int,
    _uchar, _ushort, _ulong, _ulonglong, _uint,
    _float, _double, _longdouble,
    _size_t, _ptr_diff,
    _void,
    _bool, struct_comp, _union, enum_flag, func, alias
};
...
struct CVeloxType {
    ...
    [[nodiscard]] bool is_string() const {
        return 
            (val_type == EVeloxTypeFromC::_schar || val_type == EVeloxTypeFromC::_uchar) 
            && is_pointer 
            && !is_pointer_double; 
    }
    [[nodiscard]] bool is_complex() const {
        return val_type == EVeloxTypeFromC::struct_comp 
        || val_type == EVeloxTypeFromC::_union
        || val_type == EVeloxTypeFromC::enum_flag
        || val_type == EVeloxTypeFromC::func;
    }
    [[nodiscard]] bool is_opaque() const {
        return val_type == EVeloxTypeFromC::_void && is_pointer;
    }
