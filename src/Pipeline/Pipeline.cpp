...
bool start_compilation(const std::string& target_file) {
	...
	if (out_fs->files_paths.empty()) {
		...
		std::cout << color_BLUE << "               Or start your project by creating your first script in the source folder path." << target_file << std::endl;
		return false; 
	}
	...
	if (!bind_files_info.empty()) {
		std::cout << color_YELLOW "[build] [EMBinder] [summary] " << bind_files_info.size() << color_RESET " binding files added to the main pipeline !" << std::endl;
...
void applyDefaultAndDetectNative() {
    ...
    if (COMP_CTX.target_os.empty()) {
        ...
        COMP_CTX.target_os = (pos2 != std::string::npos) ? COMP_CTX.target_arch.substr(pos1 + 1, pos2 - pos1 -1) : "unknown";
    }

    // synchroniser debug LLVM
    if (COMP_CTX.is_debug)
        COMP_CTX.LLVM_info_debug = true;
}
...
void parseArgs(int argc, char** argv) {
    for (int i = 1; i < argc; ++i) {
        std::string arg = argv[i];
        	 if (arg.rfind("--abi=", 0) == 0) 		{ COMPILATION_ARGS.insert({ "abi", arg.substr(6) }); 		COMP_CTX.target_abi = arg.substr(6); }
		...
		else if (arg == "--debug-dot-exposer") 		{ COMPILATION_ARGS.insert({ "debug-dot-exposer", "true" }); DOT_EXPOSER_PRINT = true; }
		else if (arg == "--debug-postprocessor-output") 
													{ COMPILATION_ARGS.insert({ "debug-postprocessor-output", "true" }); DEBUG_POSTPROCESSOR_OUTPUT = true; }
		...
		else if (arg.rfind("--output=", 0) == 0) { 							COMPILATION_ARGS.insert({ "output", arg.substr(9) }); 	COMP_CTX.output = std::filesystem::path(arg.substr(9)); }
		else if (arg == "--emit-obj" || arg == "--emit=obj") { 				COMPILATION_ARGS.insert({ "emit", "obj" }); 			COMP_CTX.emit_mode = CompCtx::EEmitMode::OBJ; }
