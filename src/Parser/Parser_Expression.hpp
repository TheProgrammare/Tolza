...
namespace PAR {
	...
	struct Parser_Expression {
		...
		[[nodiscard]] std::unique_ptr<AST::AExpression>									identifier(bool no_qualified_id = false, bool keyword_allowed = false);
		[[nodiscard]] std::unique_ptr<AST::Expr_ID_Generic>								identifier_typed(const std::string &name, const std::vector<std::string> &path);
        [[nodiscard]] std::unique_ptr<AST::Expression::Member_Access>					try_member_access(const std::string &name, const std::vector<std::string> &path);
        [[nodiscard]] std::unique_ptr<AST::Expression::Table_Access> 					try_table_access();
        [[nodiscard]] std::unique_ptr<AST::Expression::Call> 							function_call(const std::string &name, const std::vector<std::string> &path);
        [[nodiscard]] std::unique_ptr<AST::Expression::Call_Pipe>						function_call_pipe(const std::string &name, const std::vector<std::string> &path);
