...
std::unique_ptr<AST::AExpression> PAR::Parser_Operator::comparison()
{
  auto node = shift();
  while (ctx.tok_v.match_any(
      {TokTy::OPEN_BRACKETS, TokTy::CLOSE_BRACKETS, TokTy::OP_LOWER_EQUAL, TokTy::OP_GREATER_EQUAL})) {
    auto op    = TokTy_to_EBinOpType(ctx.tok_v.peek(-1).type);
    auto right = shift();
    node       = Create_BinOp(std::move(node), op, std::move(right));
  }
...
std::unique_ptr<AST::AExpression> PAR::Parser_Operator::try_operation() { return memory_distance(); }

std::unique_ptr<AST::Operation::Binary> PAR::Parser_Operator::Create_BinOp(std::unique_ptr<AST::AExpression> left,
                                                                           EBinOpType                        op,
...
std::unique_ptr<AST::Operation::Unary> PAR::Parser_Operator::Create_UnOp(EUnaryOpType                      op,
