...
std::shared_ptr<AST::Declaration::Global> PAR::Parser_Declaration::global_variable() {
	...
	auto kind_tok = ctx.tok_v.expect_any<65>({ TokTy::LET, TokTy::VAR, TokTy::CONST }, "Expected global variable declaration token", hint); 
...
std::shared_ptr<AST::Declaration::Generic> PAR::Parser_Declaration::generic() {
	...
	while (!ctx.tok_v.is_end()) {
		gen->targetGenericSymbols.insert(ctx.tok_v.expect_id<71>("Expected name (identifier) in generic typenames.", kHint_gen));
	...
	while (!ctx.tok_v.is_end()) {
		...
		std::string firstok = ctx.tok_v.expect_id<73>("Expected typename (identifier) in start of generic filter.", kHint_filter);

		// case: T op ...
		if (ctx.tok_v.match(TokTy::OP)) {
			auto gen_op = ctx.Create_Node<AST::Generic::Have_Op>(ctx.tok_v.peek());
			gen_op->targetGenSym = firstok;
			auto tok_op = ctx.tok_v.expect_any<74>(kOperatorTokens, "Expected operator after 'op' keyword in generic filter argument.", kHint_filter);
		...
		else ctx.tok_v.add_error<76>("Expected generic condition 'use' or 'is' in generic filter argument.", kHint_filter);
...
std::shared_ptr<AST::Declaration::Type_Alias> PAR::Parser_Declaration::type_alias() {
	...
	ctx.tok_v.expect<77>(TokTy::COLON,
