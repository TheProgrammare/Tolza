...
PAR::Parser_Context::~Parser_Context() {
	...
	delete p_loc;		p_loc	 	= nullptr;
	delete p_mem;		p_mem	 	= nullptr;
	delete p_op;		p_op	 	= nullptr;
	delete p_ref;		p_ref	 	= nullptr;
	delete p_state;		p_state	 	= nullptr;
	delete p_type;		p_type	 	= nullptr;
...
bool PAR::is_gen_args(TokenViewer& tok_v) {
	...
	size_t count = 0;
	size_t nestedBrackets = 1; // = 0 valid nested result; > 0 invalid nested result; begins at 1 because generic type args begins with < and must end by >
	...
	// check next token if is not a false positive generic args (case of interval comparaison like a < b > 0)
	if (isGenArgsValid && nestedBrackets == 0) {
		...
		if (tokAfterGenArgs.ty == TokTy::OPEN_PAREN) isAfterGenArgsValid = true;
			...
			isAfterGenArgsValid = !(
				...
				tok_v.check_any(kOperatorTokens) ||
				tok_v.check_any(kComparatorTokens));
		}
	}

	const bool result = isGenArgsValid && isAfterGenArgsValid;
	if (result)	tok_v.rewind(originPos);
	else		tok_v.rewind(originPos - 1);
...
const META::MetaInstruct *PAR::Parser_Context::get_instruct(const AST::Node &n, const std::initializer_list<std::string> &pattern) const
{
    return m_meta->get_instruct(n.get_tok_antepos(), pattern);
}

const META::MetaBlock *PAR::Parser_Context::get_metablock(const AST::Node &n, const std::initializer_list<std::string> &pattern)
{
...
/*
	...
	try {
		...
		if (tryCatchCount > 1000) {
			std::cerr << "--------------------- ! COMPILATION STOPPED ! -------------------" << std::endl;
			std::cerr << "  Compiler: Too many errors encounted (> 1000)!" << std::endl;
			std::cerr << "  Please check the code source." << std::endl;
			return;
		}
		tok_v.synchronize();
