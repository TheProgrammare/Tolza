...
std::optional<std::unique_ptr<AST::ALiteral>> PAR::Parser_Literal::try_literal(bool is_silent_error)
{
	...
	case TokTy::L_UDECIMAL:				return literal_decimal();
		// literal float
	case TokTy::L_F:					return literal_floating_point();
		// literal integer
	case TokTy::L_BIN:
	case TokTy::L_OCT:
	case TokTy::L_HEX:
	case TokTy::L_I:
	case TokTy::L_U:					return literal_integral();
		// literal character
	case TokTy::L_ASCII:				return literal_ascii();
		// literal string
	case TokTy::L_TEXTUAL:				return literal_textual();
		// literal range
	case TokTy::RANGE:			
	case TokTy::RANGE_INCLUSIVE:		return literal_range(nullptr);
		// literal collection
...
std::unique_ptr<AST::Literal::Textual_Format> PAR::Parser_Literal::literal_textual() {
	...
	while (!ctx.tok_v.is_end()) {
		if (ctx.tok_v.check(TokTy::L_TEXTUAL)) {
			...
			std::u32string val = std::wstring_convert<std::codecvt_utf8<char32_t>, char32_t>{}.from_bytes(ctx.tok_v.next().val);
			text->val = val;
...
std::unique_ptr<AST::ALiteral> PAR::Parser_Literal::literal_table() {
	static const std::string hint = "define literal table like:"
		"\n  - table { 1, 2, 3, 4 }"
	...
	if (ctx.tok_v.match(TokTy::CLOSE_BRACE))
		return ctx.Create_Node<AST::Literal::Table>(ctx.tok_v.peek(-1));;
