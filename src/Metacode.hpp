...
namespace META {
    ...
    struct MetaInstruct {
        ...
        MetaInstruct(MetaBlock* code_block, const std::vector<MetaWord>& words) 
            : code_block(code_block), words(words) {}
	...
	struct MetaScope {
        ...
        [[nodiscard]]
        bool is_valid() const { return start_scope_position < end_scope_position; }
	};
    ...
    struct MetaBlock {
        ...
        void add_children(std::unique_ptr<MetaBlock> cb) {
            ...
            tokens_to_generate.push_back(k_metacode_flag); // mark when a children must be generated before continue the rest of tokens
        }
        ...
        [[nodiscard]]
        bool is_scoped() const { return _scope.is_valid(); }
        ...
        bool is_pos_concerned(int pos) const {
            ...
            return 
                pos >= _scope.start_scope_position &&
                pos <= _scope.end_scope_position; 
        }
    ...
    struct Cond_Eq : Cond_Base {
        ...
        Cond_Eq(const std::string& ph, const std::string& val, bool isN)
            : placeholder(ph), value(val), isNot(isN) {}
        ...
        bool eval(const std::multimap<std::string, std::string>& ctx) const override {
            ...
            if (range.first == range.second)
                return isNot; // no key -> if negate: is "true"
        ...
        std::string print_eval() const override {
            if (isNot) return placeholder + " != " + value;
    ...
    struct MetaBlock_If : public MetaBlock {
        ...
        [[nodiscard]]
        MetaBlock_If *get_alternative() const { return alternative.get(); }
    ...
    struct Expand_If : public MetaBlock {
        ...
        [[nodiscard]]
        Expand_If *get_alternative() const { return alternative.get(); }
	...
	struct MetablockManager {
        ...
        void filter_metablocks_parent_cascade(std::vector<const MetaBlock*> &input) const {
            ...
            for (auto &mb : tmp_first_filter) {
                ...
                if (!mb_scope_name.empty() && mb_scopes_excluded.contains(mb_scope_name)) 
                    continue;
        ...
        const MetaInstruct* get_instruct(size_t pos, const std::initializer_list<std::string>& pattern) const {
            auto line_metas = get_metablocks_at(pos);
            for (const MetaBlock* meta : line_metas) {
                if (auto instruct = meta->match_pattern(pattern); !instruct.empty())
                    return instruct[0];
            }
            return nullptr;
        }
