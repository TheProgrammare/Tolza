...
struct Error_Diagnostic {
	...
	{
		return "[" + ESeverity_to_color(severity) + Phase_to_code(phase) + std::format("{:04}", code) + color_RESET "] ";
	}
	...
	std::string print_line() const 
	{
		...
		// Trim code
		if (token.span.line - 1 < 0 || token.span.line - 1 > scr_info.get_line_size()) 
		...
		size_t cursor_offset = token.span.col < token.span.size + trimSize ? 0 : token.span.col - (token.span.size + trimSize);
		std::string cursor_offset_str = std::string(cursor_offset, ' ');

		// final
		return
			line_offset_str + std::to_string(token.span.line) + " | " + trimmedLine + "\n"
