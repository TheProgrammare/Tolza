...
struct Evaluator final {
  ...
  Evaluator(std::unique_ptr<Declaration::Local::Pattern> _pattern) : pattern(std::move(_pattern)), kind(EKind::Pattern)
  {
  }

  Evaluator(std::unique_ptr<AExpression> _condition) : condition(std::move(_condition)), kind(EKind::Condition) {}
};
