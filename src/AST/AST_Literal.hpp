...
struct Textual_Element final {
  ...
  Textual_Element(std::unique_ptr<Text> text) : val(std::move(text)), kind(Kind::Text) {}
  Textual_Element(std::unique_ptr<Text_Lerp> lerp) : val(std::move(lerp)), kind(Kind::Lerp) {}
};
