...
{
    /*
    if (auto ptr = dynamic_cast<const Declaration::COP::Entity*>(&ty)) {
...
{
    if (auto ptr1 = dynamic_cast<const Declaration::COP::Entity*>(&ty)) {
        return resolved_system_sym->manage_entity(*ptr1);
    }
    else if (auto ptr2 = dynamic_cast<const Declaration::COP::Component*>(&ty)) {
