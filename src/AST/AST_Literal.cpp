...
{
    if (is_unsigned) 	inferred_type = Type::get_udeci_type();
    else				inferred_type = Type::get_deci_type();
}
...
{
	if (is_ascii)	inferred_type = Type::get_str_type();
	else 			inferred_type = Type::get_text_type();
}
