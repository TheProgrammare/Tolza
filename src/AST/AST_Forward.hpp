...
namespace AST {
    ...
    namespace Type {
        ...
        struct Primitive;				// i8-i128-isize u8-u128-usize b8-b128-bsize f32-f128-fsize bool void ascii utf32 str text
    ...
    namespace Literal {
        ...
        struct Table_Population;		// { [0..3] => 5 } -> { 5, 5, 5 } // { [0..3] => @i + 10 } -> { 10, 11, 12 }
        ...
        struct Entity;                  // CPlayer{CPosition.x= 10} // CPlayer{CPosition{x= 10, y= 10, z= 10}} // CPlayer{.CPosition{10, 10, 10}}
    }
    ...
    namespace Statement {
        struct If;						// if a == 0 {...} elif a == 1 {...} else {...} // if let Some(a) = value {...}
    ...
    namespace Memory {
        struct Move;                    // move'a
        struct New;                     // new ptr'T(val) // new ptr'T{}
        struct Del;                     // del a
        struct Val_Of_Ptr;              // val'a
        struct Addr_Of_Ref;             // addr'a
        struct Dist;                    // a <-> b
        struct Size;
        struct Align;
        struct GetBits;
