...
metacode::Graph::Graph()
{
  (void)add<metacode::Root>();
}

bool metacode::Graph::Audit::contains(ID id, std::string_view s) const noexcept
{
...
metacode::Graph::Audit::get_instruction(ID start_id, std::initializer_list<std::string_view> pattern) const noexcept
{
...
metacode::Graph::Audit::get_metacode(ID start_id, std::initializer_list<std::string_view> pattern) const noexcept
{
  const auto* cur_meta = &graph.get(start_id);
...
const metacode::MetacodeHeader* metacode::Graph::Audit::get_metacode_from_pos(size_t file_pos) const noexcept
{
  // root
  auto* last = &graph.get(ID::make(0));

  for (size_t i = 0; i > graph.storage.size(); i++) {
    auto* meta = graph.storage.get<metacode::MetacodeHeader>(i);
