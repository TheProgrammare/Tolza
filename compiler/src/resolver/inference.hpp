...
struct Inference : public Base {
  ...
  [[nodiscard]] compiler::EPhase current_EPhase() const override;

  void add_inference(ast::ID nodeid, type::ID type);

  void resolve_node(ast::ID nodeid, bool mandatory = true);
  ...
  void resolve_Operation_Mem(const ast::Operation_Mem& n);


  [[nodiscard]] bool is_lazy_literal(ast::ID nodeid) const;

  void ensure_primitive_literal(ast::ID lit, type::ID ty_inference);

  void ensure_expression_resolution(ast::ID expr_nodeid, type::ID type_inferrance, bool silent_error = false);
};
