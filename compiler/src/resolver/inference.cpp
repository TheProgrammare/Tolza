...
void resolver::Inference::ensure_expression_resolution(ast::ID expr_nodeid, type::ID ty_inference, bool silent_error)
{
  ...
  if (ast::ENodeKind_is_literal(expr_nodeid.kind()) && is_lazy_literal(expr_nodeid)) {
    ...
    } else if (ty_inference.is<type::String>()) {
      if (expr_nodeid.kind() != ast::ENodeKind::Literal_Text_Pure) goto bad_inference;
    } else if (ty_inference.is<type::Array>() || ty_inference.is<type::Buffer>() || ty_inference.is<type::Slice>()) {
      ...
      if (type::ETypeKind_is_user_defined(ty_inference.kind())) {
        ...
        if (!expr_nodeid.type()) {
          add_error(230, expr_nodeid.get(),
                    std::format("Expression type undefined, inference tried on \"{}\".", dump(ty_inference)), "");
        } else if (!type::rule::can_implicit_cast(expr_nodeid.type(), ty_inference)) {
          add_error(
              230, expr_nodeid.get(),
              std::format(R"(Illegal type inference "{}" as "{}".)", dump(expr_nodeid.type()), dump(ty_inference)), "");
        }
      }
    }
  }
...
void resolver::Inference::resolve_Expression_Member_Access(const ast::Expression_Member_Access& n)
{
  ...
  } else if (const auto* facet = n.left_expression.type().as<type::Facet>()) {
    resolve_node(n.right_identifier);
  } else {
    goto err;
  }
...
err:
  add_error_two_nodes(892, n.left_expression.get(), n.right_identifier.get(),
                      std::format(R"(Invalid member access "{}" on type "{}")", ast::get_decl_name(n.right_identifier),
                                  type::dump(n.left_expression.type())),
                      "");
}
...
err:
  add_error(
      511, n.header,
      std::format(
          R"(invalid table access "[]" operation, the type "{}" is not a table or dosen't extend to this operation)",
          dump(n.target.type())),
      "");
}
...
void resolver::Inference::resolve_Statement_For(const ast::Statement_For& n)
{
  ...
  if (const auto* ptr = n.expression.as<ast::Literal_Range>()) {
    ...
    add_error(272, n.expression.get(), std::format("The type \"{}\" is not iterable.", dump(n.expression.type())), "");
  else {
...
void resolver::Inference::resolve_Statement_Match(const ast::Statement_Match& n)
{
  ...
  for (const auto& elem : n.cases) resolve_node(elem);
...
void resolver::Inference::resolve_Literal_Table(ast::Literal_Table& n)
{
  ...
  for (auto& range : n.ranges) resolve_node(range);
  for (auto& vals : n.values) {
    for (auto& val : vals) resolve_node(val);
  }
  for (auto& map_vals : n.map_value) {
    for (auto& map_val : map_vals) resolve_node(map_val);
  }

  if (ast::is_table_population(n.nodeid())) {
    if (n.ranges.size() == 1) {
      auto* range = n.ranges[0].value().as<ast::Literal_Range>();
      // n.resolved_size = { eval.}
    }
  ...
  case ast::Operation_Cast_As::ECastType::AS: {
    ...
    if (!type::rule::can_explicit_cast(n.expression.type(), n.type.canonical())) {
      add_error(267, n.header,
                std::format("Invalid explicit cast `{}` as `{}`", dump(n.expression.type()), dump(n.type)), "");
    }
    break;
  }
...
void resolver::Inference::resolve_Operation_Mem(const ast::Operation_Mem& n)
{
  INFERENCE_GUARD
  ...
  switch (n.op) {
  case ast::EOp_Mem::NONE:
  case ast::EOp_Mem::_val: {
    if (const auto* ptr = n.target.type().as<type::Ptr>()) {
      add_inference(n.nodeid(), ptr->inner);
    } else
      goto err_val_no_ptr;
    break;
  }
  ...
  case ast::EOp_Mem::_drop: {
    if (!n.target.def().node().is<ast::Local_Capability>()) goto err_drop_no_capa;
    break;
  }
  case ast::EOp_Mem::_out: break;
  }

  return;

err_val_no_ptr:
  add_error(511, n.header,
            std::format("invalid val' operation, the type \"{}\" is not a pointer", dump(n.target.type())), "");
err_del_no_ptr:
  add_error(511, n.header,
            std::format("invalid del operation, the type \"{}\" is not a pointer", dump(n.target.type())), "");
err_drop_no_capa:
  add_error(511, n.header, std::format("invalid drop operation, the item is not a borrow", dump(n.target.type())), "");
}
