...
token::ETokenKind str_to_ETokenKind(std::string_view str)
{
  if (auto it = token::k_keywords.find(str); it != token::k_keywords.end()) return it->second;
  return token::ETokenKind::NONE;
}

bool token::str_is_identifier(std::string_view s)
{
...
bool token::isKeywordChar(char ch)
{
