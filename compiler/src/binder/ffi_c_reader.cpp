...
void ffi::C_Reader::generate_libc_wrappers() noexcept
{
  ...
  } else if (platform == common::env::EPlatform::macos || platform == common::env::EPlatform::ios) {
    wrap_headers(HEADERS_POSIX);
    wrap_headers(HEADERS_APPLE_POSIX);
    wrap_headers(HEADERS_APPLE_BSD);
    wrap_headers(HEADERS_APPLE_MACH);
  } else if (platform == common::env::EPlatform::windows) {
    wrap_headers(HEADERS_WINDOWS_CORE);
    wrap_headers(HEADERS_WINDOWS_SOCKET);
    wrap_headers(HEADERS_WINDOWS_ADVANCED);
  } else if (platform == common::env::EPlatform::freebsd) {
    wrap_headers(HEADERS_POSIX);
    wrap_headers(HEADERS_FREEBSD);
  } else if (platform == common::env::EPlatform::openbsd) {
    wrap_headers(HEADERS_POSIX);
    wrap_headers(HEADERS_OPENBSD);
  } else if (platform == common::env::EPlatform::netbsd) {
    wrap_headers(HEADERS_POSIX);
    wrap_headers(HEADERS_NETBSD);
  } else if (platform == common::env::EPlatform::dragonflybsd) {
    wrap_headers(HEADERS_POSIX);
    wrap_headers(HEADERS_DRAGONFLYBSD);
  } else if (platform == common::env::EPlatform::android) {
    wrap_headers(HEADERS_POSIX);
    wrap_headers(HEADERS_LINUX);
...
#define get_enum(_path, _enum_kind, _enum_default)                                                                     \
  magic_enum::enum_cast<_enum_kind>(common::utils::str_to_snake(tbl.at_path(_path).value_or("")))                      \
