...
#define DEF_COMPOSIT_ID(name, ...)                                                                                     \
  BASE_ID(                                                                                                             \
      ...
      [[nodiscard]] cu::ID cu() const noexcept { return cu::ID::make(static_cast<uint64_t>(id >> 32U)); }              \
                                                                                                                       \
      [[nodiscard]] uint32_t index() const noexcept { return id & MASK_32; }                                           \
                                                                                                                       \
      [[nodiscard]] static constexpr name make(cu::ID cuid, uint32_t index) noexcept {                                 \
        return name((size_t(cuid.raw()) << 32U) | index);                                                              \
      }                                                                                                                \
                                                                                                                       \
      void set_cu(cu::ID cuid) noexcept { id = (id & 0x00000000FFFFFFFFULL) | (uint64_t(cuid.raw()) << 32); }          \
                                                                                                                       \
      void set_index(uint32_t index) noexcept { id = (id & 0xFFFFFFFF00000000ULL) | uint64_t(index); }                 \
                                                                                                                       \
      __VA_ARGS__                                                                                                      \
                                                                                                                       \
  )
