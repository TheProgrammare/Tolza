...
struct Definition final {
  ...
  ast::EVisibility visibility; // resolved
...
struct Arena final {
  ...
  [[nodiscard]] ID add(ast::ID nodeid, ast::EVisibility visi) noexcept
  {
    assert(!freeze && "Pool is immutable after parsing pass");

    auto       new_id = ID::make(cuid, definitions.size());
    Definition def{.defid = new_id, .nodeid = nodeid, .visibility = visi};
  ...
  [[nodiscard]] const Definition& get(ID defid) const noexcept
  {
    ...
    assert(defid.index() < definitions.size());
    return definitions[defid.index()];
  }
