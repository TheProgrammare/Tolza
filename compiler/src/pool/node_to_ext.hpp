...
using ExtensionSet = std::unordered_set<ast::ID, ast::ID::Hash>;

const ExtensionSet& get_extensions(type::ID id) noexcept;

struct Arena {
  bool freeze = false;

  std::unordered_map<type::ID, ExtensionSet, type::ID::Hash> extensions;
  ...
  [[nodiscard]] const ExtensionSet& get_extensions(type::ID tyid) const
  {
    static const ExtensionSet empty;
