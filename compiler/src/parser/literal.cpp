...
ast::ID parser::Parser_Literal::literal_table()
{
  ...
  - map [a: 1, b: 2, c: 3]
  - map matrix[a: 1, b: 2 ; a1: 3, a1: 4 ; a2: 5, a3: 6]
  ...
  if (p.match(token::ETokenKind::R_SQUARE)) return node.nodeid();

  auto& current_dim = node.values.emplace_back();

  bool not_map_case = false;

  while (!p.is_end()) {
    current_dim.push_back(p.p_expr->parse_expression());

    if (p.match(token::ETokenKind::COLON)) {
      if (not_map_case)
        throw Parser_Exception(
            p, 192, p.peek(-1),
            "Unexpected colon ':', inconsistent literal table, colons are reserved for key val map association", hint);
      goto map;
    } else {
      not_map_case = true;
    }
    ...
    if (p.match(token::ETokenKind::SEMICOLON)) {
      current_dim = node.values.emplace_back();
      continue;
    }

    // table population detected
    if (p.match(token::ETokenKind::INJECT)) goto table_population;


    // next expression or terminated
    if (p.match_field_separator(token::ETokenKind::COMMA, token::ETokenKind::R_SQUARE)) break;
  }
...
table_population:
  // values are ranges
  node.ranges.reserve(node.values.size());

  // extract values to ranges
  for (auto& range : node.values) {
    if (range.size() > 1)
      throw Parser_Exception(p, 712, range[1].token().get(), "Only one range possible for each dimension", hint);

    node.ranges.emplace_back(range[0]);
  }

  node.values.clear();

  // can be values for each dimensions
  while (!p.is_end()) {
    current_dim = node.values.emplace_back();
    current_dim.emplace_back(p.p_expr->parse_expression());

    if (p.match_field_separator(token::ETokenKind::SEMICOLON, token::ETokenKind::R_SQUARE)) break;
  }

  return node.nodeid();

map:
  auto& current_map_val_dim = node.map_value.emplace_back();

  while (!p.is_end()) {
    current_map_val_dim.emplace_back(p.p_expr->parse_expression());
    ...
    if (p.match(token::ETokenKind::SEMICOLON)) {
      current_dim         = node.values.emplace_back();
      current_map_val_dim = node.map_value.emplace_back();
      continue;
    }
