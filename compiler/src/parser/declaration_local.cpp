...
ast::ID parser::Parser_Declaration_Local::lambda_capture()
{
  ...
  while (!p.is_end()) {
    ...
    auto& n_capa = p.add_get_node<ast::Operation_Mem>(p.peek(-1).tokid);

    switch (capa) {
    case ast::ECapability::NONE:
    case ast::ECapability::ref:  n_capa.op = ast::EOp_Mem::_ref; break;
    case ast::ECapability::mut:  n_capa.op = ast::EOp_Mem::_mut; break;
    case ast::ECapability::copy: n_capa.op = ast::EOp_Mem::_copy; break;
    case ast::ECapability::move: n_capa.op = ast::EOp_Mem::_move; break;
    }
