...
ast::ID parser::Parser_Declaration::_module()
{
  ...
  if (p.match(token::ETokenKind::L_CURLY)) {
    ...
    p.enter_scope(node.nodeid(), name);
    node.name = name;
...
ast::ID parser::Parser_Declaration::enumeration()
{
  ...
  p.enter_scope(enu.nodeid(), "enum " + std::string(enu.name));
...
ast::ID parser::Parser_Declaration::_union()
{
  ...
  p.enter_scope(_union.nodeid(), "union " + std::string(_union.name));
...
ast::ID parser::Parser_Declaration::flag()
{
  ...
  p.enter_scope(flag.nodeid(), "flag " + std::string(flag.name));
...
ast::ID parser::Parser_Declaration::call_contract()
{
  ...
  while (!p.is_end()) {
    if (p.match(token::ETokenKind::PRE)) {
      ...
      if (p.match(token::ETokenKind::ARROW)) {
        if (p.match_val("assert")) {
          n.pre_mode = ast::ECallContract::Assert;
          ...
          n.pre_mode = ast::ECallContract::Panic;
          (void)p.expect(331, token::ETokenKind::L_TEXTUAL, "expected text error",
                         "define panic like: `panic(\"my error info\")`");
          p.rewind(p.tok_v->position() - 1);
          n.pre_err = p.p_lit->try_literal();
        } else {
          n.pre_mode = ast::ECallContract::Result;
    ...
    if (p.match(token::ETokenKind::POST)) {
      ...
      if (p.match(token::ETokenKind::ARROW)) {
        if (p.match_val("assert")) {
          n.post_mode = ast::ECallContract::Assert;
          ...
          n.post_mode = ast::ECallContract::Panic;
          (void)p.expect(331, token::ETokenKind::L_TEXTUAL, "expected text error",
                         "define panic like: `panic(\"my error info\")`");
          p.rewind(p.tok_v->position() - 1);
          n.post_err = p.p_lit->try_literal();
        } else {
          n.post_mode = ast::ECallContract::Result;
...
ast::ID parser::Parser_Declaration::generic()
{
  ...
  p.enter_scope(gen.nodeid(), "generic " + std::string(gen.name));
