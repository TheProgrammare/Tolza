...
ast::ID parser::Parser_Expression::parse_expression()
{
  ...
  if (p.check_val("del") || p.check_val("move") || p.check_val("mut") || p.check_val("ref") || p.check_val("copy")
      || p.check_val("size") || p.check_val("addr") || p.check_val("val") || p.check(token::ETokenKind::AMPERSAND)
      || p.check(token::ETokenKind::OP_MULTIPLY)) {
    auto& n = p.add_get_node<ast::Operation_Mem>(p.peek().tokid);

    (void)p.next();
    (void)p.match(token::ETokenKind::TICK);

    n.op     = ast::EOp_Mem_from_str(p.peek().tokid.str());
    n.target = p.p_expr->parse_expression();
    return n.nodeid();
  }
...
ast::ID parser::Parser_Expression::getbits(ast::ID p_expr)
{
  ...
  auto& n    = p.add_get_node<ast::Expression_Table_Access>(p.peek().tokid);
  n.target   = p_expr;
  n.selector = p.p_expr->parse_expression();
  n.bit_size = ast::Expression_Table_Access::EBitSize::_64;
