...
{
  return {
      .debug_name = ast::get_debug_str(nodeid),
      .nodeid     = nodeid,
  };
}
...
bool scope::Scope::Items::add_symbol(symbol::ID sym) noexcept
{
  auto [_, success] = symbols.try_emplace(sym.get().get_name(), sym);
  return success;
}


symbol::ID scope::Scope::Items::find_symbol(std::string_view name) const noexcept
{
...
ast::ID scope::get_scope_node(ast::ENodeKind kind, scope::ID start_scpid) noexcept
{
  auto cur_scp = start_scpid;

  while (cur_scp) {
    auto nodeid = cur_scp.node();
    if (nodeid.get()->kind() == kind) return nodeid;

    if (auto p = cur_scp.parent())
      cur_scp = p;
    else
      return NO_ID;
  }
...
symbol::ID scope::find_lexical_symbol(scope::ID ctx, std::string_view name) noexcept
{
  const auto& scp = ctx.get();

  return scp.items.find_symbol(name);
}
symbol::ID scope::find_in_chain_scope(scope::ID ctx, std::string_view name) noexcept
{
  auto cu = ctx.cu();
  while (ctx) {
