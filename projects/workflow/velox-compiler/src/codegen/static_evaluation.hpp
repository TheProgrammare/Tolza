...
struct Static_Evaluator {
  ...
  std::expected<ast::ALiteral*, std::string> integral(const ast::literal::Integral& L, const ast::literal::Integral& R,
                                                      EBinOpType op);
  std::expected<ast::ALiteral*, std::string> floating(const ast::literal::Floating_Point& L,
                                                      const ast::literal::Floating_Point& R, EBinOpType op);
  std::expected<ast::ALiteral*, std::string> decimal(const ast::literal::Fixed_Point& L,
