...
public:
  ...
  EPrefixFound                  get_prefix_keyword(TokTy p_type, std::string_view p_key, std::string_view p_search);
  bool                          is_valid_prefix(char p_prefix, std::string_view p_current);
  void                          tokenize(const std::set<char>& p_exit_char);
  void                          process_escape();
