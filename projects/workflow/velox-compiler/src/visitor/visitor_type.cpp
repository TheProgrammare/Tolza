...
void Visitor_Type::visit(ast::expression::Call_Argument& n)
{
  ...
  // if variadic parameter : no type deducted on prototype
  // type inferred on expression
  if (!n.inferred_type) {
    n.inferred_type = n.expression->inferred_type;
  } else if (!n.inferred_type->is_same(*n.expression->inferred_type)) {
    error_two_lines(202, *n.inferred_type, n, "The type of the argument provided is incompatible.", "");
  }
}
