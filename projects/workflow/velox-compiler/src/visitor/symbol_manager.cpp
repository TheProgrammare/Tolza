...
void Symbols_Manager::try_add_extern_sym_to_generate(const ast::AIdentifier& sym, EExtern_Kind kind)
{
  ...
  for (auto& ext : scr_info.get_externs()) {
    ...
    if (ext->extern_references.empty()) {
      Extern_Item new_item(name, path, kind, sym);
      ext->extern_references.emplace_back(new_item);
      return;
    }

    for (auto& item : ext->extern_references) {
      if (item.kind != kind && item.id_node != &sym && item.name != name && item.scope != path) {
        Extern_Item new_item(name, path, kind, sym);
        ext->extern_references.emplace_back(new_item);
        return;
      }
