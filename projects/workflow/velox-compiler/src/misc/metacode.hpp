...
// pattern list
using LPattern = std::initializer_list<std::string>;
// context args list
using LCtx     = std::map<std::string, std::vector<std::string>>;

// one word (one token or list of alternative tokens) of instruction line
struct MetaWord {
  std::vector<Token> tokens;

  MetaWord(const Token& tok)
    : tokens({tok})
  {
  }
  MetaWord(const std::vector<Token>& tok)
    : tokens(tok)
  {
  }
...
struct MetaInstruct {
  ...
  [[nodiscard]]
  // returns the first occurence index -1 = not found
...
struct Metablock {
  ...
  [[nodiscard]]
  bool contains(const LPattern& pattern) const
  {
    for (auto& elem : _instructions) {
  ...
  [[nodiscard]]
  std::vector<const MetaInstruct*> match_pattern(const LPattern& pattern) const
  {
...
template <typename T>
concept DerivedFromMeta = std::is_base_of_v<Metablock, T>;

// AST param imitation
struct Metablock_Reuse_Param {
  ~Metablock_Reuse_Param();
...
struct Cond_Base {
  ...
  [[nodiscard]]
  virtual bool eval(const LCtx& ctx) const = 0;
  [[nodiscard]]
...
struct Cond_Eq : Cond_Base {
  ...
  // placeholders values placeholder_name == place_holder_symbol _T == "i32"
  [[nodiscard]]
  bool eval(const LCtx& ctx) const override
  {
...
struct Cond_And : Cond_Base {
  ...
  Cond_And(std::shared_ptr<Cond_Base> l, std::shared_ptr<Cond_Base> r)
    : lhs(l)
    , rhs(r)
  {
  }
...
struct Cond_Or : Cond_Base {
  ...
  Cond_Or(std::shared_ptr<Cond_Base> l, std::shared_ptr<Cond_Base> r)
    : lhs(l)
    , rhs(r)
  {
  }
...
struct Cond_Not : Cond_Base {
  ...
  Cond_Not(std::shared_ptr<Cond_Base> c)
    : child(c)
  {
  }
  [[nodiscard]]
...
struct Metablock_If : public Metablock {
  ...
  [[nodiscard]]
  bool eval_comp_args() const;

  [[nodiscard]]
  bool eval(const LCtx& ctx) const
  {
    if (condition)
...
struct Metablock_Expand : public Metablock {
  ...
  std::map<size_t, std::string>             placeholders_pos;
  ...
  {
    return placeholders.count(name) > 0;
  }
...
struct Manager {
  ...
  const Metablock* get_closest_metablock_at(size_t pos) const
  {
    ...
    // we need the back metablock because metablocks impacts the next position not the back position

    // metablocks keys represent start positions; a metablock applies from its key up to its logical end
  ...
  void filter_metablocks_parent_cascade(std::vector<const Metablock*>& input) const
  {
    ...
    for (auto& mb : tmp_first_filter) {
      ...
      if (!mb_scope_name.empty() && mb_scopes_excluded.count(mb_scope_name) > 0) continue;

      input.push_back(mb);
    }
  }

  // will try to get all metablocks who have a influence to the specified position handle the metablock scoped
  // exclusion
  [[nodiscard]]
  ...
  const Metablock* get_metablock(size_t pos, const LPattern& pattern) const
  {
  ...
  const MetaInstruct* get_instruct(size_t pos, const LPattern& pattern) const
  {
    auto line_metas = get_metablocks_at(pos);
    for (const Metablock* meta : line_metas) {
