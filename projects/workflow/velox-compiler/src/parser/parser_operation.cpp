...
ast::ID parser::Parser_Operator::_15_power()
{
  ...
  if (p.match(token::ETokenKind::OP_POWER)) {
    ...
    return Create_BinOp(left, ast::EBinOpType::_pow, right);
  }
  return left;
}
...
ast::ID parser::Parser_Operator::_1_memory()
{
  ...
  while (p.check_any({token::ETokenKind::OP_MEM_DIST, token::ETokenKind::OP_MEM_ADD, token::ETokenKind::OP_MEM_SUB})) {
    auto& dist  = p.add_get_node<ast::Operation_Binary>(p.peek().tokid);
...
ast::ID parser::Parser_Operator::assignment(ast::ID p_left)
{
  ...
  auto& assign           = p.add_get_node<ast::Operation_Assignment>(p.peek().tokid);
  assign.left            = p_left;
...
{
  if (p.check_any(token::k_op_unary)) {
    auto& unary    = p.add_get_node<ast::Operation_Unary>(p.peek().tokid);
    unary.unary_op = ast::ETokenKind_to_EUnaryOpType(p.next().kind);
...
ast::ID parser::Parser_Operator::Create_BinOp(ast::ID p_left, ast::EBinOpType p_op, ast::ID p_right)
{
...
ast::ID parser::Parser_Operator::Create_UnOp(ast::EUnaryOpType p_op, ast::ID p_base)
{
