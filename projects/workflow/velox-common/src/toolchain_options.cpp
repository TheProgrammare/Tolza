...
void common::Toolchain_Options::apply_context()
{
...
void common::init_toolchain_context()
{
  ...
  if (!fs::exists(file)) {
    ...
    fmt_template(str, {
                          {"custom_compiler_dir", std::string(dir)     },
                          {"compiler_used",       std::string(compiler)},
    });
  ...
  TOOLCHAIN_OPTIONS.compiler_used       = tbl.at_path("compiler.compiler_used").value_or("");
  TOOLCHAIN_OPTIONS.custom_compiler_dir = tbl.at_path("compiler.custom_compiler_dir").value_or("");

  bool file_valid = true;
  if (TOOLCHAIN_OPTIONS.compiler_used.empty() || !fs::exists(TOOLCHAIN_OPTIONS.compiler_used)) {
    std::cerr << "[velox:ERROR] No valid compiler used path \"" + TOOLCHAIN_OPTIONS.compiler_used + "\"";
    file_valid = false;
  }
  if (!TOOLCHAIN_OPTIONS.custom_compiler_dir.empty() && !fs::exists(TOOLCHAIN_OPTIONS.custom_compiler_dir)) {
    std::cerr << "[velox:ERROR] No valid custom compiler dir \"" + TOOLCHAIN_OPTIONS.custom_compiler_dir
                     + "\", it dosen't exist";
    file_valid = false;
  }
