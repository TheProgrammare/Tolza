...
void resolver::Inference::Inference::add_inference(const ast::Node& n, type::ID type)
{
  assert(n.nodeid && "Invalid node");
  assert(type && "Invalid type");

  const auto canon = type.canonical();
  compiler::inference.add(n.nodeid, canon);
  inference_count++;
}
...
void resolver::Inference::Inference::resolve_node(const ast::Node& n, bool mandatory)
{
#define resolve(_kind)                                                                                                 \
  case ast::ENodeKind::_kind: resolve_##_kind(*n.nodeid.as<ast::_kind>()); break

  auto mut_id = n.nodeid;

#define resolve_mut(_kind)                                                                                             \
  case ast::ENodeKind::_kind: resolve_##_kind(*mut_id.as<ast::_kind>()); break
  ...
  switch (kind) {
    ...
    resolve_mut(Literal_Integral);
    resolve_mut(Literal_Fixed_Point);
    resolve_mut(Literal_Floating_Point);
...
void resolver::Inference::ensure_expression_resolution(const ast::Node& expr, type::ID ty_inference, bool silent_error)
{
  ...
  if (ast::ENodeKind_is_literal(expr.kind()) && is_lazy_literal(expr)) {
    // primitive literal can be lazy type inference
    if (const auto* ty = ty_inference.as<type::Primitive>()) {
      const bool bad_integral =
          expr.kind() == ast::ENodeKind::Literal_Integral && !type::EPrimitiveTypeKind_is_integral(ty->primitive);
      const bool bad_floating =
          expr.kind() == ast::ENodeKind::Literal_Floating_Point && !type::EPrimitiveTypeKind_is_floating(ty->primitive);
      const bool bad_fixed =
          expr.kind() == ast::ENodeKind::Literal_Fixed_Point && !type::EPrimitiveTypeKind_is_fixed(ty->primitive);
      const bool bad_cune =
          expr.kind() == ast::ENodeKind::Literal_Cune && ty->primitive != type::EPrimitiveTypeKind::_cune;
      const bool bad_rune =
          expr.kind() == ast::ENodeKind::Literal_Rune && ty->primitive != type::EPrimitiveTypeKind::_rune;

      if (bad_integral && bad_floating && bad_fixed && bad_cune && bad_rune) goto bad_inference;

    } else if (auto* ty = ty_inference.as<type::String>()) {
      if (expr.kind() != ast::ENodeKind::Literal_Text_Pure) goto bad_inference;
    } else if (auto* ty = ty_inference.as<type::Array>()) {
      if (expr.kind() != ast::ENodeKind::Literal_Table) goto bad_inference;
    ...
    add_inference(expr, ty_inference);
    ensure_primitive_literal(expr.nodeid, ty_inference);
  } else {
    resolve_node(expr);
  }
  ...
  if (!expr.nodeid.is_inferred() && !silent_error) {
    add_error(234, expr, "Impossible to define the expression type.", "");
    return;
  }
...
bad_inference:
  ...
  if (expr.nodeid.type() != ty_inference && !silent_error) {
    if (type::ETypeKind_is_user_defined(ty_inference.kind())) {
      const auto* n = ty_inference.def().node().get();
      add_error_two_nodes(230, expr, *n, "Illegal type inference.", "");
    } else {
      if (!expr.nodeid.type()) {
        add_error(230, expr, "Expression type undefined, inference tried on \"" + ty_inference.dump() + "\".", "");
      } else if (!type::rule::can_implicit_cast(expr.nodeid.type(), ty_inference)) {
        add_error(230, expr,
                  "Illegal type inference \"" + expr.nodeid.type().dump() + "\" as \"" + ty_inference.dump() + "\".",
                  "");
      }
    }
  }
...
bool resolver::Inference::is_lazy_literal(const ast::Node& n) const
{
  ...
  if (const auto* ptr = n.nodeid.as<ast::Literal_Table>()) return !ptr->type;
...
void resolver::Inference::resolve_Symbol_Id(const ast::Symbol_Id& n)
{
  ...
  assert(ast::ENodeKind_is_declaration(def_node.get()->kind()) && "the node must be a declaration");
  add_inference(n, def_node.type());
}
...
void resolver::Inference::resolve_Global_Variable(const ast::Global_Variable& n)
{
  ...
  if (!n.type) {
    ...
    add_inference(n, n.expression.type());
    return;
  }

  if (n.expression) ensure_expression_resolution(*n.expression.get(), n.type);
...
void resolver::Inference::resolve_Local_Parameter(const ast::Local_Parameter& n)
{
  ...
  if (n.default_value) ensure_expression_resolution(*n.default_value.get(), n.type);
...
void resolver::Inference::resolve_Expression_Invocation(const ast::Expression_Invocation& n)
{
  ...
  for (size_t i = 0; i < proto->params.size(); i++) {
    ...
    assert(arg);
    ensure_expression_resolution(*arg->expression.get(), param.type);
  }
...
{
  if (!n.is_else) ensure_expression_resolution(*n.evaluator.get(), type::TYPEID_bool);
  resolve_node(n.codeblock);
...
void resolver::Inference::resolve_Statement_For(const ast::Statement_For& n)
{
  ...
  if (const auto* ptr = n.expression.as<ast::Literal_Range>()) {
    add_inference(*n.index.get(), ptr->start.type());
    ...
    if (n.index) add_inference(*n.index.get(), type::get_inner(n.expression.type()));
    for (auto it : n.items) add_inference(*it.get(), type::get_inner(n.expression.type()));
  }
  resolve_node(n.codeblock);
}
...
{
  ensure_expression_resolution(*n.evaluator.get(), type::TYPEID_bool);
  resolve_node(n.codeblock);
}
...
void resolver::Inference::resolve_Statement_Return(const ast::Statement_Return& n)
{
  ...
  if (!n.value) {
    if (n.returnable.type() != type::TYPEID_u0)
      add_error_two_nodes(246, n, *n.returnable.get(),
                          "Invalid void return, expression needed, the function returns values.", "");

    add_inference(n, n.returnable.type());
    return;
  }

  ensure_expression_resolution(*n.value.get(), n.returnable.type());
...
void resolver::Inference::resolve_Literal_NullPtr(const ast::Literal_NullPtr& n)
{
  ...
  add_inference(n, type::TYPEID_opaque);
}
...
void resolver::Inference::resolve_Literal_Integral(ast::Literal_Integral& n)
{
  ...
  case type::EPrimitiveTypeKind::_ssize: add_inference(n, type::TYPEID_ssize); break;
  case type::EPrimitiveTypeKind::_s8:    add_inference(n, type::TYPEID_s8); break;
  case type::EPrimitiveTypeKind::_s16:   add_inference(n, type::TYPEID_s16); break;
  case type::EPrimitiveTypeKind::_s32:   add_inference(n, type::TYPEID_s32); break;
  case type::EPrimitiveTypeKind::_s64:   add_inference(n, type::TYPEID_s64); break;
  case type::EPrimitiveTypeKind::_s128:  add_inference(n, type::TYPEID_s128); break;
  case type::EPrimitiveTypeKind::_usize: add_inference(n, type::TYPEID_usize); break;
  case type::EPrimitiveTypeKind::_u8:    add_inference(n, type::TYPEID_u8); break;
  case type::EPrimitiveTypeKind::_u16:   add_inference(n, type::TYPEID_u16); break;
  case type::EPrimitiveTypeKind::_u32:   add_inference(n, type::TYPEID_u32); break;
  case type::EPrimitiveTypeKind::_u64:   add_inference(n, type::TYPEID_u64); break;
  case type::EPrimitiveTypeKind::_u128:  add_inference(n, type::TYPEID_u128); break;
...
void resolver::Inference::resolve_Literal_Floating_Point(ast::Literal_Floating_Point& n)
{
  ...
  case type::EPrimitiveTypeKind::_fsize: add_inference(n, type::TYPEID_fsize); break;
  case type::EPrimitiveTypeKind::_f16:   add_inference(n, type::TYPEID_f16); break;
  case type::EPrimitiveTypeKind::_f32:   add_inference(n, type::TYPEID_f32); break;
  case type::EPrimitiveTypeKind::_f64:   add_inference(n, type::TYPEID_f64); break;
  case type::EPrimitiveTypeKind::_f80:   add_inference(n, type::TYPEID_f80); break;
  case type::EPrimitiveTypeKind::_f128:  add_inference(n, type::TYPEID_f128); break;
  default:                               assert(false);
  }
}
void resolver::Inference::resolve_Literal_Fixed_Point(ast::Literal_Fixed_Point& n)
{
  ...
  case type::EPrimitiveTypeKind::_dsize:  add_inference(n, type::TYPEID_dsize); break;
  case type::EPrimitiveTypeKind::_d32:    add_inference(n, type::TYPEID_d32); break;
  case type::EPrimitiveTypeKind::_d64:    add_inference(n, type::TYPEID_d64); break;
  case type::EPrimitiveTypeKind::_d128:   add_inference(n, type::TYPEID_d128); break;
  case type::EPrimitiveTypeKind::_udsize: add_inference(n, type::TYPEID_udsize); break;
  case type::EPrimitiveTypeKind::_ud32:   add_inference(n, type::TYPEID_ud32); break;
  case type::EPrimitiveTypeKind::_ud64:   add_inference(n, type::TYPEID_ud64); break;
  case type::EPrimitiveTypeKind::_ud128:  add_inference(n, type::TYPEID_ud128); break;
  default:                                assert(false);
  }
}
...
void resolver::Inference::resolve_Literal_Text_Pure(const ast::Literal_Text_Pure& n)
{
  ...
  switch (n.text_type) {
  case type::ETextType::_cstr: add_inference(n, type::TYPEID_cstr); return;
  case type::ETextType::NONE:
  case type::ETextType::_cune:
  case type::ETextType::_str:  add_inference(n, type::TYPEID_str); return;
  case type::ETextType::_rune:
  case type::ETextType::_text: add_inference(n, type::TYPEID_text); return;
  }
}
...
void resolver::Inference::resolve_Operation_Cast_As(const ast::Operation_Cast_As& n)
{
  ...
  ensure_expression_resolution(*n.expression.get(), n.type, true);
  ...
  case ast::Operation_Cast_As::ECastType::AS: {
    ...
    if (!type::rule::can_explicit_cast(n.expression.type(), n.type.canonical())) {
      add_error(267, n, "Invalid explicit cast \"" + n.expression.type().dump() + "\" as \"" + n.type.dump() + "\"",
                "");
    }
    break;
  }
