...
bool Lexer::tokenize_spec()
{
  ...
  default:   {
    auto error = Error_Diagnostic<151>(
        ...
        ::Config::EPhase::lexer, EErrorSeverity::error, {}, "Unexpected format specifier character",
...
void Lexer::add_error(const std::string& msg, const std::string& hint)
{
  ...
  std::string out =
      Error_Diagnostic<Code>(scr_info, tok, {}, ::Config::EPhase::lexer, EErrorSeverity::error, {}, msg, hint)
          .print_error();
