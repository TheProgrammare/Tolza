                                                                      ...
                                                                      const ast::literal::Integral& R, EBinOpType op)
{
  ...
  if (EPrimType_is_integral(L.type)) {
    ...
    case EBinOpType::Mod: {
      if (is_signed)
        return to_lit(L_val.srem(R_val));
      else
        return to_lit(L_val.urem(R_val));
    }
    case EBinOpType::Quo: {
      if (is_signed) {
    ...
    case EBinOpType::Rem: {
      if (is_signed) {
        ...
        return to_lit(L_val.udiv(R_val));
      }
    }
    ...
    case EBinOpType::Pow: {
      ...
      Int128 base = L_val;
      Int128 exp  = R_val;
      while (!exp.val.isZero()) {
        if (exp.val[0]) // bit de poids faible
          result.val *= base.val;
        exp.val = exp.val.lshr(1); // diviser exp par 2
        base.val *= base.val;      // exponentiation rapide
      }
    ...
    case EBinOpType::_b_nand: {
      return to_lit(~(L_val & R_val) & mask);
    }
    case EBinOpType::_or:
    ...
    case EBinOpType::ls0: {
      return to_lit(L_val.shl(R_val.getLimitedValue()));
    }
    case EBinOpType::ls1: {
    ...
    case EBinOpType::rs0: {
      return to_lit(L_val.lshr(R_val.getLimitedValue()));
    }
    case EBinOpType::rs1: {
      unsigned width = L_val.getBitWidth();
      unsigned shift = R_val.getLimitedValue();
    ...
    case EBinOpType::rsa: {
      return to_lit(L_val.ashr(R_val.getLimitedValue()));
    }
    case EBinOpType::lr: {
      return to_lit(L_val.rotl(R_val.getLimitedValue()));
    }
    case EBinOpType::rr: {
      return to_lit(L_val.rotr(R_val.getLimitedValue()));
    }
    default: {
...
{
  auto to_lit = [](const llvm::APInt& value, size_t max_integral, size_t max_decimal, bool is_unsigned) {
    return new ast::literal::Decimal(value, max_integral, max_decimal, is_unsigned);
  };
  auto to_bool = [](bool value) { return new ast::literal::Boolean(value); };
