...
struct Type {
  ...
  {
    return base_type == EType::_str || base_type == EType::_text
           || ((base_type == EType::_schar || base_type == EType::_uchar) && is_pointer && !is_pointer_double);
  }
...
struct Prototype {
  ...
  std::vector<std::tuple<EPassMode, Type, bool>> params;
};
...
[[nodiscard]] EPassMode   type_to_passMode(const Type& ty);
[[nodiscard]] std::string EPassMode_to_str(EPassMode pm);
