...
std::vector<Token> META::MetaBlock::generate_tokens(ScriptInfo& scr_info) const
{
  ...
  for (auto pos : tokens_to_generate) {
    ...
    if (pos == k_metacode_flag) {
      ...
      if (!child_toks.empty()) {
        result.insert(result.end(), child_toks.begin(), child_toks.end());
      }
...
bool META::MetaBlock_If::eval_comp_args() const
{
  ...
  if (map.empty()) {
    for (auto& [def, val] : COMPILATION_ARGS) {
      map[def] = {val};
    }
  }
...
std::vector<Token> META::MetaBlock_Expand::generate_tokens(ScriptInfo& scr_info) const
{
  ...
  for (size_t i = 0; i < tokens_to_generate.size(); i++) {
    ...
    if (pos == k_metacode_flag) {
      ...
      if (!child_toks.empty()) {
        model.insert(model.end(), child_toks.begin(), child_toks.end());
      }
          ...
          for (auto& exp_if_tok : exp_if_model) {
            if (exp_if_tok.type == ETokenType::S_METACODE_PLACEHOLDER) {
              ...
              if (auto it = combo.find(exp_if_tok.val); it != combo.end()) {
                ...
                throw std::runtime_error("Unexpected placeholder name (" + tok.val
                                         + ") the naming prevention seems to have failed");
              }
            }
          }

          result.insert(result.end(), exp_if_model.begin(), exp_if_model.end());
        }
      }
