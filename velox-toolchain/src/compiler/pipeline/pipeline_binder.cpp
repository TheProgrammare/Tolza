...
#include "compiler/binder/c_binder.hpp"
#include "pipeline.hpp"
#include "compiler/binder/ffi-json_reader.hpp"
...
bool generate_binds(const std::vector<ffi::Bind_Package>& binds)
{
  ...
  if (!start_compilation(COMP_CTX.get_binding_dir())) {
...
{
  std::filesystem::create_directories(COMP_CTX.get_ffi_json_dir());
  std::vector<std::filesystem::path> json_files;

  try {
    for (const auto& entry : std::filesystem::directory_iterator(COMP_CTX.get_ffi_json_dir())) {
  ...
  for (auto& json_f : json_files) {
    ...
    fs::path path = COMP_CTX.get_ffi_json_dir() / ast.bind.lang / ast.bind.lib;
...
bool pipeline_start_binder(const std::vector<std::shared_ptr<ScriptInfo>>& scr_infos)
{
  ...
  for (auto& scr_info : scr_infos) {
    ...
    std::filesystem::create_directories(COMP_CTX.get_binding_dir());
    size_t bind_count = 0;

    for (const auto& extern_imp : scr_info->get_externs()) {
      ffi::Bind_Package bind;
      fs::path          path = COMP_CTX.get_binding_dir() / extern_imp->name / extern_imp->extern_lib;
