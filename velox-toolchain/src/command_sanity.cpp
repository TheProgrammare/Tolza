...
{
  if (verbose) std::cout << "[velox-toolchain] Checking workspace sanity..." << std::endl;

  if (!fs::exists(ws_path)) {
    if (verbose) std::cerr << "[sanity] [error] The directory not found at " << ws_path << std::endl;
    if (verbose) std::cerr << "[velox-toolchain] The sanity check is aborted." << ws_path << std::endl;
    return false;
  }
    ...
    if (verbose)
      std::cout << "[sanity] Sanity check passed\n"
                ...
                << "\nResult:"
                   ...
                   "\n[velox-toolchain] The project is healthy and ready for compilation and development."
                << std::endl;
    return true;
  } else {
    if (verbose) {
